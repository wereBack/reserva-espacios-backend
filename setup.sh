...
install_docker_compose() {
    ...
    # Verificar e instalar Docker Buildx (requerido por compose build)
    log_info "Verificando Docker Buildx..."
    
    if docker buildx version &> /dev/null; then
        BUILDX_VERSION=$(docker buildx version | grep -oP 'v\d+\.\d+' | head -1 | tr -d 'v')
        BUILDX_MAJOR=$(echo $BUILDX_VERSION | cut -d. -f1)
        BUILDX_MINOR=$(echo $BUILDX_VERSION | cut -d. -f2)
        
        # Verificar si es >= 0.17
        if [[ "$BUILDX_MAJOR" -eq 0 && "$BUILDX_MINOR" -lt 17 ]]; then
            log_warning "Docker Buildx version $BUILDX_VERSION es muy antigua, actualizando..."
            install_buildx
        else
            log_success "Docker Buildx ya esta instalado (v$BUILDX_VERSION)"
        fi
    else
        log_info "Docker Buildx no encontrado, instalando..."
        install_buildx
    fi
}
...
install_buildx() {
    ...
    log_success "Docker Buildx instalado correctamente"
