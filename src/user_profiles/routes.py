...
def get_my_profile():
    ...
    try:
        ...
        if profile:
            return jsonify(
                {"status": "success", "profile": profile.to_dict(), "is_complete": profile.is_complete()}
            ), 200
        else:
...
def check_profile_complete():
    ...
    try:
        ...
        return jsonify({"status": "success", "is_complete": is_complete, "missing_fields": missing_fields}), 200
